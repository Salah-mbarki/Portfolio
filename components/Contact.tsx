"use client";

import { useReveal } from "@/Hooks/Usereveal";

const contactLinks = [
  {
    icon: "mail",
    label: "Email",
    display: "mbarkisalah145@gmail.com",
    href: "mailto:mbarkisalah145@gmail.com",
  },
  {
    icon: "phone",
    label: "Phone",
    display: "+216 29 227 023",
    href: "tel:+21629227023",
  },
  {
    icon: "github",
    label: "GitHub",
    display: "github.com/salah-mbaki",
    href: "https://github.com/salah-mbaki",
  },
  {
    icon: "linkedin",
    label: "LinkedIn",
    display: "linkedin.com/in/salah-mbaki",
    href: "https://www.linkedin.com/in/salah-mbarki/",
  },
];

const icons: Record<string, React.ReactNode> = {
  mail: (
    <svg className="w-5 h-5 stroke-current flex-shrink-0" viewBox="0 0 24 24" fill="none" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <rect width="20" height="16" x="2" y="4" rx="2" />
      <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
    </svg>
  ),
  phone: (
    <svg className="w-5 h-5 stroke-current flex-shrink-0" viewBox="0 0 24 24" fill="none" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.15 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.07 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21 16.92z" />
    </svg>
  ),
  github: (
    <svg className="w-5 h-5 stroke-current flex-shrink-0" viewBox="0 0 24 24" fill="none" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
      <path d="M9 18c-4.51 2-5-2-7-2" />
    </svg>
  ),
  linkedin: (
    <svg className="w-5 h-5 stroke-current flex-shrink-0" viewBox="0 0 24 24" fill="none" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
      <rect width="4" height="12" x="2" y="9" />
      <circle cx="4" cy="4" r="2" />
    </svg>
  ),
};

export default function Contact() {
  const ref = useReveal();

  return (
    <section id="contact" className="px-12 py-24 bg-surface border-t border-border">
      {/* Header */}
      <div ref={ref} className="reveal flex items-center gap-6 mb-16">
        <span className="font-mono text-xs text-accent tracking-widest">04</span>
        <h2
          className="font-display font-light text-cream"
          style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)" }}
        >
          Contact
        </h2>
        <div
          className="flex-1 h-px"
          style={{ background: "linear-gradient(to right, #2a2a38, transparent)" }}
        />
      </div>

      <div className="grid md:grid-cols-2 gap-20 items-start">
        {/* Left */}
        <div className="reveal reveal-delay-1">
          <h3
            className="font-display font-light text-cream leading-tight mb-6"
            style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)" }}
          >
            Let&apos;s build something{" "}
            <em className="text-accent italic">great</em>{" "}
            together.
          </h3>
          <p className="text-sm text-muted2 leading-loose">
            Open to new opportunities, collaborations, and interesting engineering
            challenges. Whether you have a project in mind or just want to connect
            — feel free to reach out.
          </p>
        </div>

        {/* Links */}
        <div className="reveal reveal-delay-2 flex flex-col">
          {contactLinks.map(({ icon, label, display, href }) => (
            <a
              key={label}
              href={href}
              target={href.startsWith("http") ? "_blank" : undefined}
              rel={href.startsWith("http") ? "noopener noreferrer" : undefined}
              className="group flex items-center gap-4 py-5 border-b border-border text-cream hover:text-accent transition-colors duration-200"
            >
              {icons[icon]}
              <span className="text-sm">{display}</span>
              <span className="font-mono text-[11px] tracking-widest uppercase text-muted group-hover:text-accent transition-colors duration-200 ml-auto">
                {label}
              </span>
            </a>
          ))}
        </div>
      </div>
    </section>
  );
}